...
\date{August 31, 2025 (revised \today)}
...
\begin{center}
    \begingroup\small
\noindent © 2025--2026 Sean Sowden. All Rights Reserved.
\par\endgroup
...
In Module~1, the Fold law was defined through fold-time $\bar{\tau}$, fold density $\Phi$, and threshold $\kappa$. Module~2.1 extends this framework by testing whether fold-time invariance holds across distinct dynamical systems. We compare two minimal models: a driven wave system and a frictional block--spring ``quake toy.'' These models share no common medium, yet both accumulate structural resistance as fold-time until collapse at $\kappa$. If normalized fold-time cycles remain invariant across such disparate systems, this would support the claim that the Fold law is structural and universal, not medium-specific. To test this claim, we compare these two toy systems under the same fold-time logic and assess whether normalized collapse cycles remain invariant.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Field Models}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Wave system}
The first model is a driven, damped wave equation on a discrete grid. It is not intended to reproduce the physics of a specific medium; rather, it provides a minimal system in which gradients accumulate structural resistance. The model evolves as
\begin{equation}
...
This abstraction allows the wave system to act as a generic test bed for fold dynamics.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Although conceptually different from the wave system, the quake toy follows the same fold-time logic: accumulation, threshold, collapse, reset.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \label{fig:quake_tau}
  \end{subfigure}
  \caption{Normalized fold-time traces $\bar{\tau}(t)$ for the wave system and quake toy. Both accumulate until $\bar{\tau}\geq\kappa$, then reset.}
  \label{fig:tau_traces}
\end{figure}
\FloatBarrier
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Setup} Using the wave system and stick–slip quake toy of Figure 1 above, we now test whether normalized fold-time cycles coincide across systems, dimensions, and parameters.

\section{Results}

\subsubsection{Direct comparison}
Both models produced fold-time traces $\bar{\tau}(t)$ cycling between accumulation and collapse.
When normalized to unit cycle length, the traces from the wave system and quake toy aligned closely.
...
\begin{figure}[htbp]
  ...
  \caption{Normalized fold-time $\bar{\tau}$ for the wave system (solid) and quake toy (dashed). Shaded band shows pointwise difference $\Delta$. Across 18 valid runs: median $r=0.9994$ (IQR 0.9989–0.9998), RMSE $< 2.1\times 10^{-3}$.}
...
The wave system was simulated in one, two, and three spatial dimensions.
...
This demonstrates that fold-time invariance is not a one-dimensional artifact but holds across higher-dimensional configurations of the wave system.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.60\textwidth]{mod2p1_dimensional_overlay.png}
  \caption{Dimensional overlay: 1-D, 2-D, and 3-D wave system cycles overlap within numerical precision.}
...
\begin{enumerate}
  \item \textbf{Cross-system agreement.} The wave system and stick--slip toy
  ...
  \item \textbf{Dimensional stability.} Extending the wave system from one to
...
reappears across distinct models (the wave system and stick–slip toy), across
...
they are not intended as direct physical analogs of the Fold Field, nor do they
instantiate the full ontology of Module~1. Rather, they serve to demonstrate
that fold-time invariance arises under minimal assumptions about the underlying
medium. Future work will extend these tests to experimental data and more complex
simulations, to probe the scope and limits of fold-time invariance.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendix
%-------------------------------------------------------------
...
valid if both the wave system and quake toy exhibited at least three collapse
...
plt.plot(u, srf,  lw=2, label="Wave system")
plt.plot(u, quake,lw=2, ls="--", label="Quake toy")
plt.fill_between(u, srf, quake, alpha=0.25, label="Delta")
