...
transport ($J_{ij} \propto m_i/(m_i + |\Delta m|)$, saturation form) prevents
...
\textbf{Locality constraint (Gemini/Grok audit, April 2026).}
Pre-geometric locality additionally requires that $J_{ij}$
depend only on local state variables $(m_i, m_j, \kappa)$ ---
not on global quantities such as $m_{\max}$ (the maximum
structural mass on any node in the lattice).
The canonical locally-compliant closure is the saturation form:
\begin{equation}
  J_{ij} = -D(m_j - m_i)\cdot\frac{m_i}{m_i + |m_j - m_i|},
  \label{eq:grad_transport}
\end{equation}
where $D$ is the diffusion coefficient.
At small gradients ($|m_j-m_i| \ll m_i$) this reduces to
standard diffusion. At large gradients it saturates to zero,
protecting shell boundaries from erasure.

The perturbative $\kappa$-scaled form
$J_{ij} = -D(m_j-m_i)(1 - \alpha_J|m_j-m_i|/\kappa)$
is the leading-order expansion with $\kappa$ as the local
scale and is also locally compliant~\cite{transport_deriv}.

This is fold-consistent: $J_{ij}$ depends on $m_i$ and $m_j$
(local state variables) and $\kappa$ (fold primitive),
not on spatial coordinates or global lattice properties.
It is also Module~2.2-required: any transport law violating
this property violates the Anti-Zeno bound.
Numerical confirmation on a $20^3$ lattice with three
locally compliant forms (saturation, $\kappa$-scaled,
local-rational) gives $C = 1.020 \pm 0.002$,
spread $\Delta C = 0.00110$ --- confirming that the
fixed point is a property of the locally admissible class,
not of any specific form~\cite{transport_deriv}.
...
When $\taubar_i$ exceeds $\kappa$, the accumulated fold-time
represents stored structural tension at node $i$.
The collapse releases this tension: $\taubar_i$ resets toward zero,
and the induced inflow from neighboring nodes reflects relaxation
of local $\taubar$ gradients --- mass flows toward the site of
tension release, analogous to gravitational collapse along a
potential gradient.
The pull fraction $p$ parameterizes the efficiency of this
tension-to-mass transfer; deriving $p$ from first principles
(from the $\taubar$ gradient dynamics of Module~2.2) is
identified as an open problem and does not affect the
qualitative shell formation result.
...
\textit{Resolved numerically (with seeded initial conditions).}
On a $40^3$ lattice ($N=64{,}000$ nodes, 80{,}000 steps) with
two-scale initial perturbations (one outer seed, three inner seeds),
fold dynamics produce stable nested shell structure satisfying all
...
\textit{Note on spontaneity:} the single-shell result (Result~1)
requires no seed. The multi-shell result uses seeded initial
conditions to place inner shells at controlled locations.
Whether multi-shell hierarchy emerges spontaneously from noise
on a sufficiently large lattice is not yet demonstrated and
...
$J_{ij} = -D\Delta m_{ij}\cdot m_i/(m_i+|\Delta m_{ij}|)$
\hfill (saturation transport, local \& derived)\\[1pt]
$r^* = d/(1+\sqrt{A_2/A_1})$
