...
\date{September 2025 (revised \today) \\[1ex]
© 2025--2026 Sean Sowden. All Rights Reserved}
...
Module 2.1 established that fold-time invariance holds across disparate systems---a driven wave system and a stick--slip quake toy---with cycle correlations at the $10^{-3}$ level. This invariance suggested that the fold primitives defined in Module~1---fold density $\Phi$, fold-time $\bar{\tau}$, and threshold $\kappa$---are sufficient to characterize structural dynamics independent of substrate.
...
The Fold Field $\mathcal{F}$ is the minimal closed update operator sufficient to evolve
fold-time under transport, accumulation, and threshold/reset:
\[
\mathcal{F}: \{x_i(t),\,\bar{\tau}_i(t),\,\Phi_i(t)\}
\;\mapsto\;
\{x_i(t+1),\,\bar{\tau}_i(t+1),\,\Phi_i(t+1)\}.
\]

\medskip
\noindent\textit{State space.}
$x_i(t)$ is the generalized state at node $i$; $\bar{\tau}_i(t) \geq 0$ is the
accumulated Fold-Time; $\Phi_i(t) \geq 0$ is the Fold Density (local structural loading).
These are the quantities that evolve under $\mathcal{F}$. The evolution of $\Phi$ is
closure-dependent: it may be transported directly, inferred via $m = \Phi\bar{\tau}$,
or updated through event redistribution. It does not possess an independent primary
evolution equation; its role at any given step depends on the admissible closure in use.

\medskip
\noindent\textit{Parameters.}
$J_{ij}$ are transport couplings (define network topology); $\kappa > 0$ is the threshold.
These define the regime in which $\mathcal{F}$ operates; they are not state variables.

\medskip
\noindent\textit{Operators.}
$\mathcal{F}$ decomposes as the composition $\mathcal{F} = R \circ A \circ T$,
where $T$ is transport, $A$ is accumulation, and $R$ is threshold/reset, applied in that order:

\medskip
\noindent\textbf{(1) Transport} ($T$).
\[
x_i'(t) = x_i(t) + \sum_j J_{ij}\bigl(x_j(t) - x_i(t)\bigr).
\]
State differences redistribute across the network via coupling weights.

\medskip
\noindent\textbf{(2) Accumulation} ($A$).
\[
\bar{\tau}_i'(t) = \bar{\tau}_i(t) + A_i\bigl(x'(t)\bigr),
\]
where $A_i(\cdot)$ is the local accumulation functional satisfying
$A_i \geq 0$ for all inputs and $A_i = 0$ at equilibrium.
This guarantees $\bar{\tau}$ is monotonically non-decreasing between threshold events
and that no spurious accumulation occurs at a fixed point.
Local mismatch accumulates as Fold-Time.

\medskip
\noindent\textbf{(3) Threshold / Reset} ($R$).
\[
\bar{\tau}_i(t+1) =
\begin{cases}
0 & \text{if } \bar{\tau}_i'(t) \ge \kappa, \\
\bar{\tau}_i'(t) & \text{otherwise,}
\end{cases}
\qquad
x_i(t+1) =
\begin{cases}
R_i\!\bigl(x'(t)\bigr) & \text{if } \bar{\tau}_i'(t) \ge \kappa, \\
x_i'(t) & \text{otherwise,}
\end{cases}
\]
\[
\Phi_i(t+1) =
\begin{cases}
\Phi_i^{\,\mathrm{reset}} & \text{if } \bar{\tau}_i'(t) \ge \kappa, \\
\Phi_i(t) & \text{otherwise,}
\end{cases}
\]
where $R_i(\cdot)$ is the reset map and $\Phi_i^{\,\mathrm{reset}}$ is the
post-collapse redistribution of Fold Density (admissible: $\Phi_i^{\,\mathrm{reset}} \geq 0$,
total budget conserved). Threshold crossing triggers a discrete structural update
and simultaneous Fold-Time and Fold Density reset.
\end{definition}

\noindent The Fold Field is not a physical field or substrate. It is an evaluation rule
that operates on system state variables and defines their discrete-time evolution.
Equivalently, the full map takes the compact form
\[
(x,\,\bar{\tau},\,\Phi)_{t+1} = \mathcal{F}(x,\,\bar{\tau},\,\Phi)_t,
\]
with a continuous phase (transport $+$ accumulation) and a discontinuity
(threshold-triggered reset). All emergent structure arises from repeated application
of $\mathcal{F}$.

\medskip
\noindent\textit{Plain statement.} The Fold Field is the smallest rule that turns a
system into a dynamical structure: things influence each other (transport), differences
build up locally (accumulation), and when buildup crosses a limit the system updates
(threshold/reset). No medium, no hidden layer, no extra physics---just:
\textbf{if} accumulated tension $\ge$ threshold \textbf{then} reset/update,
\textbf{else} keep accumulating.
